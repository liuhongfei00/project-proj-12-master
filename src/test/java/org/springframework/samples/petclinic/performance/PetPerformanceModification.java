...
public class PetPerformanceModification {

	@Autowired
	MockMvc mvc;

	final MockHttpServletRequestBuilder createBuilder = post("/owners/1/pets/1/edit").param("name", "Lightning")
		.param("birthDate", "2014-10-11").param("type", "horse");
	...
	static void setup(@Autowired MockMvc mvc) {
		try {
			...
			mvc.perform(post("/owners/1/pets/1/edit").param("name", "Lightning").param("birthDate", "2014-10-11").param("type",
				"horse"));

			writer = new PrintWriter("pet_modification_performance.csv");
